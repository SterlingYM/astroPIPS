        ...
        if isinstance(star, photdata):
            if star.period:
                if star.epoch:
                    ...
                    popt,pcov = fit_cphi(star.x - star.epoch % star.period, star.y, star.period, K=4) #epoch may cause error depending on implementation
                    ...
                    print("Run get_epoch to determine the epoch necessary for calculation")
            else:
...
class Cacciari2005(StellarModels):
    """Subclass for StellarModels corresponding to _________'s paper from ____"""
    # C_1,C_2,C_3,C_4 = self.fit_terms[0],self.fit_terms[1],self.fit_terms[2],self.fit_terms[3]
    # phi21,phi31,phi41 = self.fit_terms[4], self.fit_terms[5], self.fit_terms[6]
    # period = self.period
